...
async fn main() -> io::Result<()> {
    ...
    if let Err(e) = api::create_queue(&msg_channel, "viewed").await {
        eprintln!("Error in creating message queue: {e}");
    }
    ...
    if let Err(e) = tokio::spawn(async move {
        if let Err(e) = api::consume_viewed_msg(consumer_channel).await {
